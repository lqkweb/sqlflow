...
_lr_productions = [
  ...
  ('dml -> train','dml',1,'p_dml','parser.py',49),
  ('dml -> register','dml',1,'p_dml','parser.py',50),
  ('dml -> load','dml',1,'p_dml','parser.py',51),
  ('dml -> save','dml',1,'p_dml','parser.py',52),
  ('dml -> connect','dml',1,'p_dml','parser.py',53),
  ('dml -> set','dml',1,'p_dml','parser.py',54),
  ('utility -> exit','utility',1,'p_utility','parser.py',59),
  ('utility -> print','utility',1,'p_utility','parser.py',60),
  ('showtables -> SHOW TABLES','showtables',2,'p_showtables','parser.py',65),
  ('createuser -> CREATE USER ID PASSWORD STRING','createuser',5,'p_createuser','parser.py',70),
  ('grantuser -> GRANT power_list ON non_mrelation_list TO non_mrelation_list','grantuser',6,'p_grantuser','parser.py',75),
  ('revokeuser -> REVOKE power_list ON non_mrelation_list FROM non_mrelation_list','revokeuser',6,'p_revokeuser','parser.py',80),
  ('power_list -> power_list , power_type','power_list',3,'p_power_list','parser.py',85),
  ('power_list -> power_type','power_list',1,'p_power_list','parser.py',86),
  ('power_type -> SELECT','power_type',1,'p_power_type','parser.py',94),
  ('power_type -> UPDATE','power_type',1,'p_power_type','parser.py',95),
  ('power_type -> INSERT','power_type',1,'p_power_type','parser.py',96),
  ('power_type -> DELETE','power_type',1,'p_power_type','parser.py',97),
  ('power_type -> PRINT','power_type',1,'p_power_type','parser.py',98),
  ('power_type -> ALL','power_type',1,'p_power_type','parser.py',99),
  ('alerttable -> ALERT TABLE ID ADD attrtype','alerttable',5,'p_alerttable','parser.py',105),
  ('alerttable -> ALERT TABLE ID DROP non_mrelation_list','alerttable',5,'p_alerttable','parser.py',106),
  ('createtable -> CREATE TABLE ID ( non_mattrtype_list )','createtable',6,'p_createtable','parser.py',114),
  ('createindex -> CREATE INDEX ID ( ID )','createindex',6,'p_createindex','parser.py',119),
  ('droptable -> DROP TABLE ID','droptable',3,'p_droptable','parser.py',124),
  ('dropindex -> DROP INDEX ID ( ID )','dropindex',6,'p_dropindex','parser.py',129),
  ('print -> PRINT ID','print',2,'p_print','parser.py',134),
  ('exit -> EXIT','exit',1,'p_exit','parser.py',139),
  ('query -> SELECT non_mselect_clause FROM non_mrelation_list opwhere_clause oplimit_clause opas_clause','query',7,'p_query','parser.py',144),
  ('insert -> INSERT INTO ID VALUES inservalue_list','insert',5,'p_insert','parser.py',149),
  ('inservalue_list -> ( non_mvalue_list ) , inservalue_list','inservalue_list',5,'p_inservalue_list','parser.py',154),
  ('inservalue_list -> ( non_mvalue_list )','inservalue_list',3,'p_inservalue_list','parser.py',155),
  ('delete -> DELETE FROM ID opwhere_clause','delete',4,'p_delete','parser.py',163),
  ('update -> UPDATE ID SET relattr EQ relattr_or_value opwhere_clause','update',7,'p_update','parser.py',168),
  ('train -> TRAIN non_mselect_clause opas_clause','train',3,'p_train','parser.py',173),
  ('register -> REGISTER non_mselect_clause opas_clause','register',3,'p_register','parser.py',178),
  ('load -> LOAD non_mselect_clause opas_clause','load',3,'p_load','parser.py',183),
  ('save -> SAVE OVERWRITE TABLE opas_clause','save',4,'p_save','parser.py',188),
  ('connect -> CONNECT JDBC opwhere_clause','connect',3,'p_connect','parser.py',193),
  ('set -> SET non_mselect_clause opas_clause','set',3,'p_set','parser.py',198),
  ('non_mattrtype_list -> attrtype , non_mattrtype_list','non_mattrtype_list',3,'p_non_mattrtype_list','parser.py',203),
  ('non_mattrtype_list -> attrtype','non_mattrtype_list',1,'p_non_mattrtype_list','parser.py',204),
  ('attrtype -> ID type','attrtype',2,'p_attrtype','parser.py',212),
  ('attrtype -> ID type ( NUMBER )','attrtype',5,'p_attrtype','parser.py',213),
  ('attrtype -> PRIMARY KEY ( ID )','attrtype',5,'p_attrtype','parser.py',214),
  ('type -> INT','type',1,'p_type','parser.py',224),
  ('type -> CHAR','type',1,'p_type','parser.py',225),
  ('non_mselect_clause -> non_mrelattr_list','non_mselect_clause',1,'p_non_mselect_clause','parser.py',230),
  ('non_mselect_clause -> *','non_mselect_clause',1,'p_non_mselect_clause','parser.py',231),
  ('non_mrelattr_list -> relattr , non_mrelattr_list','non_mrelattr_list',3,'p_non_mrelattr_list','parser.py',236),
  ('non_mrelattr_list -> relattr','non_mrelattr_list',1,'p_non_mrelattr_list','parser.py',237),
  ('relattr -> ID . ID','relattr',3,'p_relattr','parser.py',245),
  ('relattr -> ID','relattr',1,'p_relattr','parser.py',246),
  ('non_mrelation_list -> relation , non_mrelation_list','non_mrelation_list',3,'p_non_mrelation_list','parser.py',254),
  ('non_mrelation_list -> relation','non_mrelation_list',1,'p_non_mrelation_list','parser.py',255),
  ('relation -> ID','relation',1,'p_relation','parser.py',263),
  ('opwhere_clause -> WHERE non_mcond_list','opwhere_clause',2,'p_opwhere_clause','parser.py',268),
  ('opwhere_clause -> nothing','opwhere_clause',1,'p_opwhere_clause','parser.py',269),
  ('oplimit_clause -> LIMIT value','oplimit_clause',2,'p_oplimit_clause','parser.py',275),
  ('oplimit_clause -> nothing','oplimit_clause',1,'p_oplimit_clause','parser.py',276),
  ('opas_clause -> AS ID','opas_clause',2,'p_opas_clause','parser.py',282),
  ('opas_clause -> nothing','opas_clause',1,'p_opas_clause','parser.py',283),
  ('non_mcond_list -> non_mcond_list AND non_mcond_list','non_mcond_list',3,'p_non_mcond_list','parser.py',289),
  ('non_mcond_list -> non_mcond_list OR non_mcond_list','non_mcond_list',3,'p_non_mcond_list','parser.py',290),
  ('non_mcond_list -> ( non_mcond_list )','non_mcond_list',3,'p_non_mcond_list','parser.py',291),
  ('non_mcond_list -> condition','non_mcond_list',1,'p_non_mcond_list','parser.py',292),
  ('condition -> relattr op relattr_or_value','condition',3,'p_condition','parser.py',302),
  ('condition -> relattr EQ null_value','condition',3,'p_condition','parser.py',303),
  ('condition -> relattr NE null_value','condition',3,'p_condition','parser.py',304),
  ('relattr_or_value -> relattr','relattr_or_value',1,'p_relattr_or_value','parser.py',309),
  ('relattr_or_value -> value','relattr_or_value',1,'p_relattr_or_value','parser.py',310),
  ('non_mvalue_list -> value , non_mvalue_list','non_mvalue_list',3,'p_non_mvalue_list','parser.py',315),
  ('non_mvalue_list -> value','non_mvalue_list',1,'p_non_mvalue_list','parser.py',316),
  ('non_mvalue_list -> null_value , non_mvalue_list','non_mvalue_list',3,'p_non_mvalue_list','parser.py',317),
  ('non_mvalue_list -> null_value','non_mvalue_list',1,'p_non_mvalue_list','parser.py',318),
  ('value -> STRING','value',1,'p_value_string','parser.py',326),
  ('value -> NUMBER','value',1,'p_value_number','parser.py',331),
  ('null_value -> NULL','null_value',1,'p_null_value','parser.py',336),
  ('op -> LT','op',1,'p_op','parser.py',341),
  ('op -> LE','op',1,'p_op','parser.py',342),
  ('op -> GT','op',1,'p_op','parser.py',343),
  ('op -> GE','op',1,'p_op','parser.py',344),
  ('op -> EQ','op',1,'p_op','parser.py',345),
  ('op -> NE','op',1,'p_op','parser.py',346),
  ('nothing -> <empty>','nothing',0,'p_nothing','parser.py',351),
]
